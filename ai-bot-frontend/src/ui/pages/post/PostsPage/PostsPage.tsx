    ...
    return (
        <Box>
            ...
            {!loading && (!posts || posts.content.length === 0) && (
                <Typography color='text.secondary'>No extracted posts yet. Run an extraction session first.</Typography>
            )}
            {!loading && posts && posts.content.length > 0 && (
                <>
                    ...
                    {posts.totalPages > 1 && (
                        <Box sx={{ display: 'flex', justifyContent: 'center', mt: 3 }}>
                            <Pagination count={posts.totalPages} page={page + 1} onChange={(_, value) => setPage(value - 1)}/>
                        </Box>
                    )}
                </>
            )}
